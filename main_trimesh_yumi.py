...
def run_assembler(n_stages=16,top_k=[float("inf")], rollout_depth=0, start_from=None, folder='results', reverse=False):
    ...
    #JSON logging
    root_folder = sim_folder.parent
    robot_moves = []
    
    def save_robot_moves():
        out_path = root_folder / "robot_moves.json"
        with out_path.open('w') as f:
            json.dump(robot_moves, f, indent=2)
        print(f"Saved {len(robot_moves)} robot moves to {out_path}")
    ...
    for stage in range(start_from, n_stages):
        ...
        best_pid, best_vec = print_best_move_info(scored_moves)
        # --- log move to JSON (ADDED) ---
        robot_moves.append({
            'stage': int(stage),
            'piece_id': int(best_pid),
            'dx': float(best_vec[0]),
            'dy': float(best_vec[1]),
            'dz': float(best_vec[2]),
        })
